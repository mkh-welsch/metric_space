...
gaussian_blur(size_t n)
{
    ...
    namespace detail {
        ...
        template <typename T>
        static auto test_vec_of_vec(int) ->
            sfinae_true<decltype(std::declval<T>()[0][0])>;
        template<typename>
...
auto SSIM<V>::operator()(const Container &img1, const Container &img2) const -> distance_type
{
    ...
    //for (auto it1 = img1.begin(), it2 = img2.begin(); it1 != std::distance(img1.begin()+img1.size() - n + 1) || it2 != std::distance(img2.begin()+img2.size() - n + 1); ++it1, ++it2)
