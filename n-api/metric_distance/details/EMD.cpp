...
// reenabled by Dmitry V, because of simple_example.cpp use it.
...
struct emd_impl
{
        ...
        )
    {
        ...
        else if (std::is_floating_point<T>::value)
        {
            ...
            for (size_t i = 0; i < N; ++i)
            {
                sumP += static_cast<double>(POrig[i]);
                sumQ += static_cast<double>(QOrig[i]);
                for (size_t j = 0; j < N; ++j)
                {
                    if (C[i][j] > imaxC)
                        imaxC = C[i][j];
...
// EMD<Container>::operator()(const Container &Pc,
//                            const Container &Qc,
